...
func parseIssues(issues Issues, startPoint time.Time) float64 {
	...
	return (totalClosureTime / 24.0) / issueCounter
}
...
func parseCommits(commits []Commit, startPoint time.Time) (float64, int) {
	if len(commits) == 0 {
		return 0, 0
	}
	...
	return totalCommits / timeFrame, len(contributorMap)
}
...
func parseReleases(releases []Release, LatestRelease time.Time, startPoint time.Time) (float64, float64) {
	if len(releases) == 0 {
	...
	return time.Since(LatestRelease).Hours() / 24.0 / 7.0, releaseCounter / timeFrame
...
func CalculateActivityScore(repoInfo *RepoInfo, startPoint time.Time) (float64, float64) {
	avgIssueClosureTime := parseIssues(repoInfo.Issues, startPoint)
	commitCadence, contributors := parseCommits(repoInfo.Commits, startPoint)
	ageLastRelease, releaseCadence := parseReleases(repoInfo.Releases, repoInfo.LatestRelease, startPoint)
	...
	repoScore := (commitWeight * commit_score) +
		(contributorWeight * contributer_score) +
		(releaseWeight * release_score) +
		(issueWeight * issue_score)

	depScore := 0.0
	// dep_score := ((commitWeight * commit_score) +
	// 	(contributorWeight * contributer_score) +
	// 	(releaseWeight * release_score) +
	// 	(issueWeight * issue_score)) / (num_dep)

	activityScore := (repoWeight * repoScore) + (depWeight * depScore)

	return 100 * activityScore, 100
}
