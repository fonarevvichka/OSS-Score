...
aws lambda update-function-configuration --function-name queryScore --handler main \
                                --timeout 300 --environment "Variables={GIT_PAT_1=$GIT_PAT_1, GIT_PAT_2=$GIT_PAT_2, GIT_PAT_3=$GIT_PAT_3, GIT_PAT_4=$GIT_PAT_4, GIT_PAT_5=$GIT_PAT_5, MONGO_URI=$MONGO_URI, SHELF_LIFE=$SHELF_LIFE, QUERY_QUEUE=$DEP_QUERY_QUEUE, DYNAMODB_TABLE=$DYNAMODB_TABLE}" \
